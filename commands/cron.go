...
func (c *CronCommand) Synopsis() string {
	return "Run the cron scheduler with spawner and notifier"
}
...
func (c *CronCommand) Run(args []string) int {
	...
	scheduler.Start()
	logger.Info(fmt.Sprintf("Cron scheduler started with %d entries", scheduler.Entries()))
...
func (c *CronCommand) projectLoader() func() ([]internal.CronProject, error) {
	if c.configDir != "" {
